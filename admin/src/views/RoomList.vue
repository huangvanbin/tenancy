<template>
	<div>
		...
		<el-table :data="items.slice((currentPage - 1) * pagesize, currentPage * pagesize)" height="450" style="width: 100%">
			...
			<el-table-column prop="monthly" label="租金"></el-table-column>
			
			<el-table-column label="操作" fixed="right" width="180">
			  <template slot-scope="scope">
			    <el-button size="small" @click="$router.push(`/rooms/edit/${scope.row._id}`)">编辑</el-button>
	...
	export default {
		...
		methods: {
			...
			async remove(row){
				...
				await this.$http.delete(`rest/rooms/${row._id}`)
					this.$message({
						type: 'success',
						message: '删除成功!'
					});
